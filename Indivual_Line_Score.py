...
def Line_Score(Line, Dict):
    ...
    def Pounds_For_Meter(Line_Length, cost_per_meter=1):  # Value from Cost Metric.py
        return Line_Length * cost_per_meter

    def Road_Distance(start_coords, end_coords):
        import path_generation
        # Get distance, make sure units are kilometers, and return float
        # gmap = googlemaps.Client(key=api_key)
        # route = gmap.directions(start_coords, end_coords, avoid=[
        #                         'highways', 'tolls'])[0]
        ...
        # get distance from file in km
        handler = path_generation.Data_Management()
        route = handler.load_route(start_coords, end_coords)
    ...
    for Val in Dict.values():
        Coords = Val[1]
        Weight = sum(Val[0])
        min_distance = math.inf    # just needs to be big
        ...
        # count += Weight
        Scores.append(Score(min_distance)*Weight)
    # Find mean score
    ...
    for i, Station in enumerate(Line):
        ...
        Actual_Distance = Road_Distance(Prev_Station, Station)
...
if __name__ == '__main__':
    ...
    Line_2 = ["E00073342", "E00073396", "E00074002",
              "E00174242", "E00174242", "E00174312"]
