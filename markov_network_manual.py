...
lowest_tram_stops = merged_tram_stops_gdf.nsmallest(22, "total_arrivals")
...
# Ensure OA_code is set as index
tram_stops = tram_stops.set_index("OA_code")

# Identify the lowest 10 commuter count tram stops
lowest_tram_stops = tram_stops.nsmallest(27, "total_arrivals").index.tolist()  # Convert to a list of OA_codes

# Debugging: Print lowest tram stops
print(f"🚀 Black Nodes (Lowest 10 Stops): {lowest_tram_stops}")
...
# Step 1: Add manual tram lines (edges)
manual_edges = [
...
# Step 2: Plot tram stops (Black for lowest 10, Red for others)
for oa_code, row in tram_stops.iterrows():
    color = "black" if oa_code in lowest_tram_stops else "red"  # Black for lowest 10, red otherwise
